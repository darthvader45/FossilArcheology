...
public class DinoAIFlying extends EntityAIBase {
	...
	//  CONSTANTS
	// =============
	public enum eFlyingState { FS_ON_GROUND, FS_IN_AIR, FS_ASCENDING, FS_DESCENDING };
	...
	//  CONSTRUCTOR
	// =============
	public DinoAIFlying( EntityFlyingDino _Owner )
	{
	...
	private boolean SetTakeOffData()
	{
		...
		/*
		for( int i = 0; i < c_nTakeOffHeightInBlocks; ++i )
		{
			if( g_bDebugFlying == false )
			{
				// TODO: Find out if there are blocks above us.
				return false;
			}
		}
		*/
		...
		if( m_fsCurrentState != eFlyingState.FS_ASCENDING ) // Really just a paranoid check.
		{
	...
	{
		//m_edDinoAgent.setPosition(m_edDinoAgent.posX, m_edDinoAgent.posY, m_edDinoAgent.posZ);
