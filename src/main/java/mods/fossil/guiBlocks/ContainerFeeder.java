    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int getSlot)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
    		...
    		if (getSlot > HERB_INPUT) // if it's not in the INPUT and in player inventory
    		{
    			...
    			if (EnumDinoFoodItem.foodtype(itemstack1.itemID) == EnumDinoFoodItem.ISCARNIVOROUS)
    			{
    			...
    			if (EnumDinoFoodItem.foodtype(itemstack1.itemID) == EnumDinoFoodItem.ISHERBIVOROUS)
    			{
    				// try to place in either Input slot; add 1 to final input slot because mergeItemStack uses < index
